...
public class CustomInMemoryTokenStore extends InMemoryTokenStore {
  ...
  public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication){
		String portInformation = corsFilter.getRemoteAddress()+"-"+corsFilter.getRemotePort()+"-"+corsFilter.getRemoteBrowser();
		OAuth2AccessToken accessToken = super.getAccessToken(authentication);
		if(accessToken!=null && customAccessTokenStore.get(accessToken.getValue())!=null && !customAccessTokenStore.get(accessToken.getValue()).equals(portInformation)) {
			if(continueAccessTokenStore.get(accessToken.getValue())!=null && !customAccessTokenStore.get(accessToken.getValue()).equals(continueAccessTokenStore.get(accessToken.getValue()))) {
